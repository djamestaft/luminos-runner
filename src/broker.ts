...
export interface LocalJob { jobId: string; project: string; profile: string; state: JobState; sessionId: string; branch: string; commitSha?: string; prUrl?: string; }
...
export interface HerdrAdapter { create(input:{jobId:string;cwd:string;profile:string}):Promise<{sessionId:string}>; prompt(sessionId:string,text:string,timeoutMs:number):Promise<"working"|"ready"|"unknown">; status(sessionId:string):Promise<"working"|"ready"|"unknown">; read(sessionId:string,maxLines:number):Promise<string>; }
export interface GitWorkspaceAdapter { create(input:{jobId:string;policy:ProjectPolicy}):Promise<{branch:string;cwd:string}>; handoff?(input:{job:LocalJob;policy:ProjectPolicy}):Promise<{commitSha:string;prUrl:string}>; }
...
export class HostBroker {
  ...
  private async dispatch(command:BrokerCommand):Promise<BrokerResult>{
    ...
    if(command.verb==="recover_job"){if(job.state!=="unknown")return this.result(job,"refused");job.state=await this.herdr.status(job.sessionId);await this.registry.put(job);return this.result(job,job.state==="unknown"?"unknown":"ok");}
    if(command.verb==="close_job"){if(job.state!=="handoff_ready")return this.result(job,"refused");job.state="closed";await this.registry.put(job);return this.result(job,"ok");}
