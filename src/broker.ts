...
import { isHerdrCreateDiagnostic } from "./herdrErrorDiagnostics.js";
...
export type HandoffPhase = "validate"|"local_sha"|"remote_branch"|"push"|"pr_lookup"|"pr_create"|"pr_validate"|"registry_write";
export interface LocalJob { jobId: string; project: string; profile: string; state: JobState; sessionId: string; workspaceId?: string; branch: string; commitSha?: string; prUrl?: string; handoffPhase?: HandoffPhase; remoteSha?: string; }
...
export interface GitWorkspaceAdapter { create(input:{jobId:string;policy:ProjectPolicy}):Promise<{branch:string;cwd:string}>; handoff?(input:{job:LocalJob;policy:ProjectPolicy;checkpoint?:(evidence:Partial<Pick<LocalJob,"commitSha"|"remoteSha"|"prUrl">>&{handoffPhase:HandoffPhase})=>Promise<void>}):Promise<{commitSha:string;prUrl:string}>; cleanup?(input:{job:LocalJob;policy:ProjectPolicy}):Promise<void>; }
...
export class HostBroker {
  ...
  public async execute(raw:unknown):Promise<BrokerResult>{
    ...
    catch(error){const message=error instanceof Error?error.message:"";const busy=message==="busy";const uncertain=!busy&&(mutating.has(command.verb)||message.includes("registry"));if(uncertain){try{const job=await this.registry.get(command.jobId);if(job){job.state="unknown";await this.registry.put(job);}}catch{/* The result remains unknown when even durable reconciliation state cannot be written. */}}const bounded=/^(?:git_(?:remote|fetch|base|worktree|process_start)_failed|remote_(?:mismatch|lookup_failed|conflict)|branch_mismatch|uncommitted_changes|missing_job_commit|invalid_commit|pr_(?:lookup_failed|ambiguous|mismatch|create_failed|validate_failed|invalid_response)|herdr_(?:protocol|command|create|process_start|process_start_failed_enoent)_failed|herdr_process_start_failed(?:_enoent)?|process_(?:timeout|output_too_large)|invalid_registry|registry_write_failed)$/.test(message);const safe=bounded||isHerdrCreateDiagnostic(message)?message:"operation_failed";return{contractVersion:CONTRACT_VERSION,jobId:command.jobId,state:busy?"refused":uncertain?"unknown":"failed",category:busy?"busy":uncertain?"unknown":"failed",summary:busy?"busy":safe};}
  }
  ...
  private async dispatch(command:BrokerCommand):Promise<BrokerResult>{
    ...
    if(command.verb==="handoff_job"){if(job.state==="handoff_ready")return this.result(job,"ok");if(!["ready","unknown"].includes(job.state)||!this.git.handoff)return this.result(job,"refused");const policy=this.policies.get(job.project);if(!policy)return this.result(job,"refused");const checkpoint=async(evidence:Partial<Pick<LocalJob,"commitSha"|"remoteSha"|"prUrl">>&{handoffPhase:HandoffPhase})=>{Object.assign(job,evidence);await this.registry.put(job);};const handoff=await this.git.handoff({job,policy,checkpoint});job.commitSha=handoff.commitSha;job.prUrl=handoff.prUrl;job.handoffPhase="registry_write";job.state="handoff_ready";await this.registry.put(job);return this.result(job,"ok");}
    if(command.verb==="recover_job"){if(job.state==="handoff_ready")return this.result(job,"ok");if(job.state!=="unknown")return this.result(job,"refused");if((job.commitSha||job.remoteSha||job.prUrl||job.handoffPhase)&&this.git.handoff){const policy=this.policies.get(job.project);if(!policy)return this.result(job,"refused");const checkpoint=async(evidence:Partial<Pick<LocalJob,"commitSha"|"remoteSha"|"prUrl">>&{handoffPhase:HandoffPhase})=>{Object.assign(job,evidence);await this.registry.put(job);};const handoff=await this.git.handoff({job,policy,checkpoint});job.commitSha=handoff.commitSha;job.prUrl=handoff.prUrl;job.handoffPhase="registry_write";job.state="handoff_ready";await this.registry.put(job);return this.result(job,"ok");}job.state=await this.herdr.status(job.sessionId);await this.registry.put(job);return this.result(job,job.state==="unknown"?"unknown":"ok");}
