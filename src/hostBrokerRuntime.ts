...
export const broker=new HostBroker(new Map([[policy.project,policy]]),new FileJobRegistry(path.join(required("BROKER_STATE_ROOT"),"jobs.json")),new GitCliWorkspaceAdapter(executor),new HerdrCliAdapter(executor,process.env.BROKER_AGENT_KIND==="pi"?"pi":"codex","herdr",process.env.BROKER_SHELL_ZDOTDIR));
