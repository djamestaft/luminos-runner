import path from "node:path";import{stat}from"node:fs/promises";import type{GitWorkspaceAdapter,LocalJob,ProjectPolicy}from"./broker.js";import type{ProcessExecutor}from"./processExecutor.js";
...
export class GitCliWorkspaceAdapter implements GitWorkspaceAdapter{
  public constructor(private executor:ProcessExecutor,private git="git",private gh="gh",private githubToken?:string){}
  public async create({jobId,policy}:{jobId:string;policy:ProjectPolicy}){const remote=(await this.phase("git_remote_failed",()=>this.runGit(policy,["remote","get-url",policy.remote]))).trim();if(remote!==policy.expectedRemoteUrl)throw new Error("remote_mismatch");await this.phase("git_fetch_failed",()=>this.runGit(policy,["fetch","--prune",policy.remote]));const baseSha=(await this.phase("git_base_failed",()=>this.runGit(policy,["rev-parse","--verify",`${policy.baseRef}^{commit}`]))).trim();const id=safeJob(jobId),branch=`jobs/${id}`,cwd=path.join(policy.worktreeRoot,id);const existing=await stat(cwd).catch((error:NodeJS.ErrnoException)=>error.code==="ENOENT"?undefined:Promise.reject(error));if(existing){if(!existing.isDirectory())throw new Error("git_worktree_failed");await this.validateReusableWorktree(cwd,branch,baseSha);return{branch,cwd};}await this.phase("git_worktree_failed",()=>this.runGit(policy,["worktree","add","-b",branch,cwd,policy.baseRef]));return{branch,cwd};}
  ...
  private async validateReusableWorktree(cwd:string,branch:string,baseSha:string):Promise<void>{await this.phase("git_worktree_failed",async()=>{const actualBranch=(await this.command(this.git,["-C",cwd,"branch","--show-current"])).trim();const head=(await this.command(this.git,["-C",cwd,"rev-parse","HEAD"])).trim();const dirty=(await this.command(this.git,["-C",cwd,"status","--porcelain"])).trim();if(actualBranch!==branch||head!==baseSha||dirty)throw new Error("not_reusable");});}
  ...
  private async command(file:string,args:string[],startCode="git_process_start_failed",exitCode="git_operation_failed"){let out;try{const environment=file===this.gh&&this.githubToken?{...process.env,GH_TOKEN:this.githubToken}:undefined;out=await this.executor.run(file,args,300_000,environment);}catch{throw new Error(startCode);}if(out.exitCode!==0)throw new Error(exitCode);return out.stdout;}
}
